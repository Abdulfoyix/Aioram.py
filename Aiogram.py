...
async def start(mess:types.Message):
    await mess.answer("Siz Adminsiz")


@dp.message_handler(IsPrivate(),commands=['start'])
async def start(mess:types.Message):
    await mess.answer("Siz shaxsiy chat",reply_markup=kontent_keyboard)
...
async def start(mess:types.Message):
    await mess.answer("Siz gurppadasiz")
...
#fayllar fotolar
@dp.message_handler(IsPrivate(), text="1-rasm")
...
async def new_member(message: types.Message):
    members = ", ".join([m.username for m in message.new_chat_members])
